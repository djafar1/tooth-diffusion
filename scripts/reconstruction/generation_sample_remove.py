...
def main():
    ...
    # --- Filter settings ---
    NUM_SAMPLES = args.num_samples   # already in args
    ...
    TEETH_SELECTION_MODE = "manual"   # "manual" or "random"
    MANUAL_TEETH_TO_REMOVE = [1, 16, 17, 32]  # 1-indexed human-readable but in the code it will be read as 0-indexed
    RANDOM_K = 4                      # used when TEETH_SELECTION_MODE == "random"
    # -------------------------------------------------------------------------

    output_dir = os.path.join(args.output_dir, "remove_teeth_by_tooth_presence")
    ...
    for sample in loader:
        ...
        # count current missing teeth in dataset label (before our modification)
        ...
        # move to device (leave name on CPU)
        base_sample = {}
        ...
        logger.log(f"Sample currently has {missing_now} missing teeth (before modification).")

        # figure out which teeth to set to 0
        present_indices_0based = (base_tp[0] > 0).nonzero(as_tuple=True)[0].tolist()  # 0-based positions
        if len(present_indices_0based) == 0:
            logger.log("No present teeth in sample; skipping.")
            continue
        
        if TEETH_SELECTION_MODE == "manual":
            teeth_to_remove_hr = MANUAL_TEETH_TO_REMOVE[:]  # 1-based
            # keep only those that are actually present
            teeth_to_remove = [t for t in teeth_to_remove_hr if (t - 1) in present_indices_0based]
            if len(teeth_to_remove) == 0:
                logger.log("Manual list contains no present teeth; skipping.")
                continue
        else:  # "random"
            k = min(RANDOM_K, len(present_indices_0based))
            chosen_0based = random.sample(present_indices_0based, k=k)
            teeth_to_remove = [i + 1 for i in chosen_0based]  # convert to 1-based

        logger.log(f"Teeth to remove (1-based): {teeth_to_remove}")

        # modify tooth_presence (set selected to 0.0)
        tooth_presence_modified = base_tp.clone()
        for t in teeth_to_remove:
            tooth_presence_modified[0, t - 1] = 0.0
        logger.log(f"Modified tooth_presence vector ready.")

        # build conditioning from the FULL image 
        full_image_th = base_image
        LLL_img, LLH_img, LHL_img, LHH_img, HLL_img, HLH_img, HHL_img, HHH_img = dwt(full_image_th)
        
        cond_dwt = th.cat([LLL_img / 3.0, LLH_img, LHL_img, LHH_img, HLL_img, HLH_img, HHL_img, HHH_img],dim=1,)

        model_kwargs = {"tooth_presence": tooth_presence_modified}

        # start from noise in wavelet space
        img = th.randn(
            args.batch_size,
            8,
            args.image_size // 2,
        ...
        # simple filename: include removed list for traceability
        remove_tag = "_".join(map(str, teeth_to_remove)) if len(teeth_to_remove) else "none"
        output_name = os.path.join(output_dir, f"tp_removed_{remove_tag}_{base_name}.nii.gz")
