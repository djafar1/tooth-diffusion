...
class GaussianDiffusion:
    ...
    def ddim_sample(
        ...
        if not sampling_steps:
            ...
            xp = np.arange(0, 1000, 1, dtype=float)
    ...
    def ddim_sample_loop_progressive(
        ...
        if sampling_steps:
            ...
            indices = tmp[:-1].round().astype(int)
            indices_prev = tmp[1:].round().astype(int)
        else:
    ...
    def training_losses(self, model, x_start, t, classifier=None, model_kwargs=None, noise=None, labels=None,
        ...
        if mode == 'default':
            ...
            LLL_n, LLH_n, LHL_n, LHH_n, HLL_n, HLH_n, HHL_n, HHH_n = dwt(noise)
            noise_dwt = torch.cat([LLL_n, LLH_n, LHL_n, LHH_n, HLL_n, HLH_n, HHL_n, HHH_n], dim=1)
